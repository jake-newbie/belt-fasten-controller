...
void SegLCD_Init(void){ //Initializes all components of SLCD on the FRDM-KL46Z
	...
	PORTB->PCR[7]  = PORT_PCR_MUX(0u);					//Set PTB7 to LCD_P7	(2D/2E/2G/2F)
	PORTB->PCR[8]  = PORT_PCR_MUX(0u);					//Set PTB8 to LCD_P8	(DP2/2C/2B/2A)
	PORTB->PCR[10] = PORT_PCR_MUX(0u);					//Set PTB10 to LCD_P10	(4D/4E/4G/4F)
	PORTB->PCR[11] = PORT_PCR_MUX(0u);					//Set PTB11 to LCD_P11	(COL/4C/4B/4A)
	PORTB->PCR[21] = PORT_PCR_MUX(0u);					//Set PTB21 to LCD_P17	(DP1/1C/1B/1A)
	PORTB->PCR[22] = PORT_PCR_MUX(0u);					//Set PTB22 to LCD_P18	(COM3)
	PORTB->PCR[23] = PORT_PCR_MUX(0u);					//Set PTB23 to LCD_P19	(COM2)
	PORTC->PCR[17] = PORT_PCR_MUX(0u);					//Set PTC17 to LCD_P37	(1D/1E/1G/1F)
	PORTC->PCR[18] = PORT_PCR_MUX(0u);					//Set PTC18 to LCD_P38	(DP3/3C/3B/3A)
	PORTD->PCR[0]  = PORT_PCR_MUX(0u);					//Set PTD0 to LCD_P40	(COM0)
	PORTE->PCR[4]  = PORT_PCR_MUX(0u);					//Set PTE4 to LCD_P52 	(COM1)
	PORTE->PCR[5]  = PORT_PCR_MUX(0u);					//Set PTE5 to LCD_P53	(3D/3E/3G/3F)
...
void SegLCD_SeatBelt(void){
	LCD->WF8B[LCD_FRONTPLANE0] = (LCD_SEG_D |  LCD_SEG_F | LCD_SEG_G);
	LCD->WF8B[LCD_FRONTPLANE1] = (LCD_SEG_A | LCD_SEG_C);
	LCD->WF8B[LCD_FRONTPLANE2] = (LCD_SEG_D | LCD_SEG_E | LCD_SEG_F | LCD_SEG_G);
	LCD->WF8B[LCD_FRONTPLANE3] = (LCD_SEG_A);
	LCD->WF8B[LCD_FRONTPLANE4] = (LCD_SEG_D | LCD_SEG_E | LCD_SEG_F | LCD_SEG_G);
	LCD->WF8B[LCD_FRONTPLANE5] = (LCD_SEG_A | LCD_SEG_C | LCD_SEG_B);
	LCD->WF8B[LCD_FRONTPLANE6] = (LCD_SEG_D | LCD_SEG_E | LCD_SEG_F | LCD_SEG_G);
	LCD->WF8B[LCD_FRONTPLANE7] = (LCD_SEG_A);
	SegLCD_Col_On();
}
...
void segLCD_Time_count(uint8_t Value){
	int b1,b2,b3,b4;
	b1 = Value / 1000;
	b2 = (Value - b1*1000)/100;
	b3 = (Value - b1*1000 - b2*100)/10;
	b4 = Value - b1*1000 - b2*100 - b3*10;
	SegLCD_Set(b1,1);
	SegLCD_Set(b2,2);
	SegLCD_Set(b3,3);
	SegLCD_Set(b4,4);
